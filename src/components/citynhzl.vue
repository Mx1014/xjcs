    ...
    <div class="xjcs-c-content">
        <div class="xjcs-c-form">
            <div class='xjcs-c-action'>
                ...
                <span class="xjcs-city-type"><span @click="changeEntype('电能')" :class="{actived:enType=='电能'}">电能</span><span @click="changeEntype('水能')" :class="{actived:enType=='水能'}">水能</span><span @click="changeEntype('气能')" :class="{actived:enType=='气能'}">气能</span><span @click="changeEntype('油能')" :class="{actived:enType=='油能'}">油能</span>
                    </span>
        ...
        <div style="position:relative;padding-bottom:20px">
            ...
            <div>
                <Table :columns="cols" :data="dataList" style="position:relative;padding-bottom:20px"></Table>
                <div class="export-data">
                     <a :href="`/Index/get_data_by_city_time?import=outexcel&intChart=${intChart}&time_type=${dateType}&energy_type=${enType}&datetime=${dateType==0?datetime.format('yyyy-MM-dd 00:00:00'):''}`" target="_blank">导出数据</a>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        buildTree(list) {
            ...
            var _buildChildren = (list) => {
                ...
                list.forEach(ele => {
                    rlist.push({
                        ...
                        value: ele.cityid,
                        expand: true,
                        children: _buildChildren(ele.regList)
                    })
                })
                return rlist
            }
            list.forEach(ele => {
                rList.push({
                    ...
                    expand: true,
                    disabled: true,
                    children: _buildChildren(ele.cityList)
                })
            })
        ...
        getData() {
            this.$http.get("/Index/get_data_by_city_time", {
                params: {
