            ...
            <div>
                <Table :columns="cols" :data="dataList"></Table>
                 <div class="export-data">
                     <a :href="`/Index/get_data_by_time?import=outexcel&intChart=${intChart}&time_type=${dateType}&energy_type=${enType}&datetime=${dateType==0?datetime.format('yyyy-MM-dd 00:00:00'):''}&emp_code=${selnode.value}`" target="_blank">导出数据</a>
                </div>
            </div>
        </div>
      </div>
        ...
        .then(res => {
          if (this.enType == "气能" || this.enType == "油能") {
            res.data = {
              ...
              energy_today_list: { name: "", data: [] },
              energy_yesterday_list: { name: "2018-01-03", data: [] },
              energy_lastyeartoday_list: null,
              energy_chartdata_list: [
                { name: "2018-01-04", data: [] },
                { name: "2018-01-03", data: [] }
              ],
              energy_department_energy_arr: [{ value: [] }]
            };
          }
...
myChart.clear()
      let series = data.energy_department_energy_arr.map(ele => {
        return {
          name: ele.name,
          data: ele.value.map(item => {
            return item.value;
          }),
          type: "line",
          smooth: true
      ...
      let option = {
        ...
        legend: {
          data: data.energy_department_energy_arr.map(ele => {
            return ele.name;
          }),
           textStyle:{
                        color,
    ...
    setRight1(data) {
      ...
      require("echarts/lib/chart/bar"); //线
      let all =
        (parseFloat(data[0]) ? parseFloat(data[0]) : 0) +
        (parseFloat(data[1]) ? parseFloat(data[1]) : 0) +
        (parseFloat(data[2]) ? parseFloat(data[2]) : 0);
      var builderJson = {
        all: all,
        charts: {
          ["今日" +
    ...
    setRight2(data) {
      ...
      var thime = this.$thime=='thime3'?"light":"default"
      let myChart = echarts.init(document.getElementById("echart-nhzl-2"),thime);
      let option = {
         tooltip: {
                    trigger: 'item',
                    formatter:(params)=> {
